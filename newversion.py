...
# ---------- Split detection: one reference text spread across multiple segments ----------
# In read-speech corpora, a speaker often pauses inside a long paragraph or
# sentence, so VAD splits the audio into 2-4 chunks that *together* cover one
# reference item. Without correction, this looks like several segments each
# labeled `paragraph (38%)`, `paragraph (44%)` and so on — confusing and wrong.
#
# This module detects such splits and assigns ordered sub-IDs (`p1`/`p2`/`p3`
# for paragraphs, `s2a`/`s2b` for split sentences). It works for both the
# Align-GT path (where labels were already assigned but a chunk got merged or
# split) and the Predict-on-GT path (where we measure how each chunk relates
# to a known reference).


def _word_range_covered(prediction, reference, prev_cursor=0):
    """Given a model prediction for a chunk and the full reference text,
    return (first_matched_idx, last_matched_idx, last_matched_cursor) showing
    which slice of the reference's word sequence this chunk appears to cover.

    Uses the same fuzzy-matching as measure_paragraph_progress but returns
    a (start, end) span instead of just an end-fraction. `prev_cursor` lets
    consecutive calls pick up where the previous chunk left off, which is
    what gives us "this chunk covers words 110-220 of the reference."
    """
    ref_words = (reference or "").split()
    pred_words = (prediction or "").split()
    if not ref_words or not pred_words:
        return None, None, prev_cursor
    pred_norm = [_norm_for_compare(w) for w in pred_words]
    ref_norm = [_norm_for_compare(w) for w in ref_words]

    first_matched = None
    last_matched = None
    cursor = prev_cursor  # position within reference where we start looking
    # Sweep predicted words and try to anchor each one in the reference,
    # advancing monotonically. Anchors closer to prev_cursor are preferred
    # so we don't jump back to an earlier part of the paragraph.
    for j, pw in enumerate(pred_norm):
        if not pw:
            continue
        # Look for the next acceptable match at or after `cursor`
        best_i = -1
        for i in range(cursor, len(ref_norm)):
            if not ref_norm[i]:
                continue
            if pw == ref_norm[i] or _fuzzy_score(pw, ref_norm[i]) >= 0.80:
                best_i = i
                break
        if best_i >= 0:
            if first_matched is None:
                first_matched = best_i
            last_matched = best_i
            cursor = best_i + 1
    return first_matched, last_matched, cursor


def detect_paragraph_splits(segments, reference_paragraph,
                            min_chunks=2, time_gap_s=20.0):
    """Look for runs of consecutive segments that together cover the
    reference paragraph, and return a relabel plan.

    A group is detected when:
      - 2+ consecutive segments are typed `paragraph` (by label or by
        classify_segment_type), AND
      - Their gap times are within `time_gap_s` of each other, AND
      - Their predictions, swept in reading order, cover non-overlapping
        consecutive spans of the reference.

    Returns: list of dicts, one per detected group:
        {
          "indices": [i, i+1, ...],         # segment indices in self.segments
          "spans": [(word_start, word_end), ...],
          "new_labels": ["p1", "p2", "p3"], # in reading order
          "ref_word_count": N,
        }
    The caller decides whether to apply the relabel.
    """
    if not reference_paragraph or len(segments) < min_chunks:
        return []

    ref_words = reference_paragraph.split()
    if len(ref_words) < 5:
        return []
    ...
    while i < n:
        ...
        # Identify a paragraph-typed segment as a potential group start.
        # Accept literal "paragraph", "p1", "p2", ... or any label that
        # classifies as paragraph by content length.
        stype = classify_segment_type(label)
        if stype != "paragraph":
            i += 1
            continue
        # Collect a contiguous run of paragraph-typed segments separated by
        # at most `time_gap_s` of silence.
        run = [i]
        j = i + 1
        while j < n:
            nxt = segments[j]
            nxt_label = (nxt.get("label") or "").strip()
            nxt_type = classify_segment_type(nxt_label)
            gap = float(nxt.get("start", 0)) - float(segments[j - 1].get("end", 0))
            if nxt_type != "paragraph" or gap > time_gap_s:
                break
            run.append(j)
            j += 1
        if len(run) >= min_chunks:
            # Check that consecutive segments cover ascending, non-overlapping
            # word spans of the reference.
            spans = []
            cursor = 0
            ok = True
            for idx in run:
                pred = (segments[idx].get("prediction") or "").strip()
                if not pred:
                    ok = False; break
                fm, lm, cursor = _word_range_covered(pred, reference_paragraph, cursor)
                if fm is None or lm is None:
                    ok = False; break
                # Each span should be a non-trivial slice (≥3 words) — otherwise
                # the model probably hallucinated a single matching word.
                if lm - fm < 2:
                    ok = False; break
                spans.append((fm, lm))
            if ok:
                # Together they should cover most of the paragraph
                total_covered = sum(e - s + 1 for s, e in spans)
                if total_covered >= 0.50 * len(ref_words):
                    groups.append({
                        "indices": run,
                        "spans": spans,
                        "new_labels": [f"p{k+1}" for k in range(len(run))],
                        "ref_word_count": len(ref_words),
                    })
        i = j if len(run) >= min_chunks else i + 1
    return groups


def detect_sentence_splits(segments, reference_sentences, time_gap_s=10.0):
    """Same idea as detect_paragraph_splits but for sentences (s1..s5).

    A split is recognised when 2+ consecutive segments carry the SAME sentence
    ID (e.g. both labeled "s2") OR carry sentence-typed labels whose
    predictions together cover one reference sentence. Returns a relabel plan
    that converts them to s2a, s2b, s2c… (alphabetic sub-IDs are unambiguous
    and easy to sort).
    """
    if not reference_sentences or len(segments) < 2:
        return []
    ...
    while i < n:
        seg = segments[i]
        label = (seg.get("label") or "").strip()
        m = _RE_SEQ_SENT.match(label)
        if not m:
            i += 1
            continue
        s_id = label.lower()  # e.g. "s2"
        ref_text = reference_sentences.get(s_id, "")
        if not ref_text:
            i += 1
            continue
        # Collect contiguous segments with the SAME sentence ID
        run = [i]
        j = i + 1
        while j < n:
            nxt_label = (segments[j].get("label") or "").strip().lower()
            gap = float(segments[j].get("start", 0)) - float(segments[j - 1].get("end", 0))
            if nxt_label != s_id or gap > time_gap_s:
                break
            run.append(j)
            j += 1
        if len(run) >= 2:
            # Letter-subscript labels: s2a, s2b, s2c, …
            new_labels = [f"{s_id}{chr(ord('a') + k)}" for k in range(len(run))]
            groups.append({
                "indices": run,
                "sentence_id": s_id,
...
class AutoSegmentThread(QThread):
    ...
    If `vocab` is provided (a list of expected labels from the transcript /
    wordlist / paste box), each chunk's free-form prediction is "snapped" to
    the closest vocab entry by fuzzy match — so the annotator sees canonical
    labels (e.g. "apple") instead of model spelling errors ("appal"). Snap is
    one-per-chunk; chunks that look like they merged multiple expected items
    are emitted with a `_merge_warning` flag so the UI can flag them.
    """
    ...
    # Threshold under which we DON'T snap — leave the raw prediction so the
    # user sees we couldn't find anything close enough in the vocab. 0.55 is
    # forgiving enough for typical ASR spelling errors but strict enough that
    # "apple" doesn't snap to "elephant".
    SNAP_THRESHOLD = 0.55

    def __init__(self, y, sr, y16, lang_iso, lang_name, vad_params,
                 do_asr=True, vocab=None):
        super().__init__()
        self.y = y
        self.sr = sr
        self.y16 = y16
        ...
        self.do_asr = do_asr
        self.vocab = list(vocab) if vocab else None  # optional snap target
        self._stop = False

    def request_stop(self):
        self._stop = True

    def _snap_to_vocab(self, pred_text, chunk_dur):
        """Find the best-matching vocab entry for the chunk's prediction.

        Returns (snapped_label, similarity, merge_warning_reason_or_None).
        Falls back to the raw prediction when no candidate is good enough.
        """
        if not self.vocab or not pred_text:
            return pred_text, 0.0, None

        # Score every vocab item against the prediction, take the best.
        # O(n_vocab) per chunk — fine for vocabs of a few hundred items.
        best_score = -1.0
        best_label = pred_text
        for v in self.vocab:
            s = _fuzzy_score(pred_text, v)
            if s > best_score:
                best_score = s
                best_label = v

        if best_score < self.SNAP_THRESHOLD:
            # No good single-item match — keep the raw prediction. But check
            # whether the prediction looks like SEVERAL vocab items concatenated
            # (which is exactly the "VAD merged a row of alphabets" case). If
            # so, surface a warning so the user knows to split this chunk.
            multi_matches = []
            pred_tokens = pred_text.split()
            if len(pred_tokens) >= 2:
                for tok in pred_tokens:
                    for v in self.vocab:
                        if _fuzzy_score(tok, v) >= 0.80:
                            multi_matches.append(v)
                            break
            if len(multi_matches) >= 2:
                # Distinct vocab items found across the tokens — definitely merged
                distinct = list(dict.fromkeys(multi_matches))[:5]
                warning = (f"prediction looks like {len(multi_matches)} vocab "
                           f"items concatenated: {', '.join(distinct)}"
                           + (" …" if len(multi_matches) > 5 else "")
                           + " — consider splitting this chunk")
                return pred_text, best_score, warning
            return pred_text, best_score, None

        # Heuristic merge-warning: did VAD likely merge multiple expected
        # items into this chunk? Two signals:
        #   1. The chunk is much longer than a typical single-token utterance
        #      AND the matched label is itself short (≤2 chars or 1 word).
        #   2. The model's prediction contains many more whitespace tokens
        #      than the snapped label (suggests the model heard multiple
        #      items but only one matched the snap target).
        warning = None
        pred_words = pred_text.split()
        label_words = best_label.split()
        # Signal 2: prediction has 2+ more tokens than the matched label
        if len(pred_words) >= len(label_words) + 2:
            warning = (f"prediction has {len(pred_words)} tokens but matched "
                       f"label '{best_label}' has {len(label_words)} — "
                       f"chunk may contain multiple labels")
        # Signal 1: short matched label + long chunk
        elif (len(best_label.replace(" ", "")) <= 2
              and chunk_dur > 1.5
              and len(pred_text) > 2 * max(1, len(best_label))):
            warning = (f"single-char label '{best_label}' in a {chunk_dur:.1f}s "
                       f"chunk — VAD may have merged adjacent characters")

        return best_label, best_score, warning
    ...
    def run(self):
        try:
            ...
            for i, ch in enumerate(chunks):
                ...
                # Decide the label. If we have a vocab, try to snap pred to
                # the closest known entry; otherwise fall back to the raw
                # prediction (legacy behavior, also used for unscripted audio).
                chunk_dur = float(ch["end"]) - float(ch["start"])
                merge_warning = None
                snap_score = None
                if self.vocab and pred:
                    snapped, snap_score, merge_warning = self._snap_to_vocab(
                        pred, chunk_dur
                    )
                    label = snapped
                else:
                ...
                # `label` is what the user will see/edit (pre-filled with
                # the snapped or raw prediction); `prediction` is the *raw*
                # model output kept verbatim so we can always show GT vs
                # PRED side-by-side, even after edits.
                out_seg = {
                    "label": label,
                ...
                if snap_score is not None:
                    out_seg["_snap_score"] = snap_score
                if merge_warning:
                    out_seg["_merge_warning"] = merge_warning
                self.segment.emit(out_seg)
...
def align_labels_grouped(chunk_preds, gt_labels,
                         per_label_threshold=0.55,
                         max_labels_per_chunk=40):
    """Many-to-one forced alignment.

    Designed for the case where the speaker reads many short items (alphabets,
    word lists) rapidly, so VAD lumps several GT labels into ONE audio chunk.
    Unlike align_labels_to_chunks (strict 1:1), this assigns a *contiguous run*
    of GT labels to each chunk.

    Strategy — a 1-D dynamic program over GT labels:
      dp[j] = best score covering the first j GT labels using the first k
      chunks (k advances as we consume chunks). We let chunk i absorb a run
      gt[a:b]; the score for that assignment is how well the chunk's predicted
      text matches the *concatenation* of gt[a:b], with a mild per-label bonus
      so the model prefers covering more labels when the text supports it.

    Returns list of dicts:
       {gt_idx, chunk_idx, n_in_chunk, pos_in_chunk, similarity}
    so the caller can sub-split each chunk's time span across its labels.
    """
    n = len(chunk_preds)
    m = len(gt_labels)
    if n == 0 or m == 0:
    ...
    norm_preds = [_norm_for_compare(p) for p in chunk_preds]
    norm_gts = [_norm_for_compare(g) for g in gt_labels]

    def run_score(chunk_i, a, b):
        """How well does chunk_i's prediction match gt[a:b] concatenated?
        Returns (score, per_label_sims_list)."""
        joined = "".join(norm_gts[a:b])
        pred = norm_preds[chunk_i]
        if not joined:
            return -1.0, []
        if not pred:
            # No transcription (common for isolated chars). Give a small
            # positive score so chars still get assigned by position rather
            # than dropped — better to place them than lose them.
            sims = [0.25] * (b - a)
            return 0.25 * (b - a) * 0.5, sims
        # Overall similarity of concatenated text
        overall = _fuzzy_score(pred, joined)
        # Per-label similarity against the chunk pred (each label should at
        # least appear somewhere in the prediction)
        sims = [_fuzzy_score(gt_labels[k], chunk_preds[chunk_i]) for k in range(a, b)]
        # Score rewards matching more labels when overall text agrees
        return overall * (b - a), sims

    NEG = float("-inf")
    # dp[i][j] = best score using first i chunks to cover first j labels
    ...
    for i in range(n + 1):
        for j in range(m + 1):
            ...
            cur = dp[i][j]
            if i < n:
                # Option A: chunk i is noise/silence — covers 0 labels
                if cur > dp[i + 1][j]:
                    dp[i + 1][j] = cur
                    back[i + 1][j] = (i, j, 0, [])
                # Option B: chunk i absorbs a run gt[j:j+r]
                max_r = min(max_labels_per_chunk, m - j)
                for r in range(1, max_r + 1):
                    sc, sims = run_score(i, j, j + r)
                    if sc <= NEG:
                        continue
                    val = cur + sc
                    if val > dp[i + 1][j + r]:
                        dp[i + 1][j + r] = val
                        back[i + 1][j + r] = (i, j, r, sims)
    # Find best end state: all labels covered (j=m), any chunk count
    best_i, best_val = None, NEG
    for i in range(n + 1):
        if dp[i][m] > best_val:
            best_val, best_i = dp[i][m], i
    if best_i is None:
        return []
    # Backtrack
    out = []
        ...
        if r > 0:
            for pos in range(r):
                ...
                out.append({
                    ...
                    "n_in_chunk": r,
                    "pos_in_chunk": pos,
                    "similarity": sims[pos] if pos < len(sims) else 0.0,
                })
        i, j = pi, pj
    out.reverse()
    return out
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Collected by Auto-Detect when chunk-merge heuristics fire; shown
        # in a popup at the end of the run so the user knows which segments
        # may have lost adjacent labels to VAD over-merging.
        self._autodetect_merge_warnings = []
    ...
    def _build_ui(self):
        ...
        # Split-detect: find consecutive segments that together cover one
        # reference paragraph/sentence and relabel them p1/p2/p3 (or s2a/s2b).
        self.btn_detect_splits = QPushButton("🧩 Detect Splits")
        self.btn_detect_splits.setStyleSheet(
            "QPushButton{font-weight:bold;color:white;background:#0d9488;"
            "border:1px solid #0f766e;padding:5px 14px;border-radius:4px}"
            "QPushButton:hover{background:#0f766e}"
            "QPushButton:disabled{color:#9ca3af;background:#f3f4f6}"
        )
        self.btn_detect_splits.setToolTip(
            "Detect when one reference paragraph or sentence got split across\n"
            "multiple audio segments (speaker paused mid-paragraph) and relabel\n"
            "them p1/p2/p3 or s2a/s2b in reading order.\n\n"
            "Run this AFTER 🎯 Predict on GT so each segment has a prediction\n"
            "to compare against the reference text."
        )
        self.btn_detect_splits.clicked.connect(self.on_detect_splits)
        r2.addWidget(self.btn_detect_splits)
    ...
    def _ask_autodetect_vocab(self):
        """Picker for Auto-Detect's snap-to-vocab feature.

        Returns one of:
            list[str]   — labels to snap each chunk's prediction to
            []          — explicit "no vocab" (free-form prediction; legacy mode)
            None        — user cancelled the dialog

        Sources offered: loaded wordlist, transcripts/<iso>.txt sentences +
        paragraph, custom pasted list, or free-form (no snapping).
        """
        iso, lang_name = self._current_lang()
        wordlist_seq = self._build_wordlist_gt_sequence()
        # Also try to surface transcripts/<iso>.txt as a separate option,
        # since it's the canonical reference (shared with app.py).
        transcripts_items = []
        try:
            txt_sents, txt_para = load_transcripts_file(iso)
            for k in sorted(txt_sents.keys(),
                            key=lambda x: int(x[1:]) if x.startswith("s") and x[1:].isdigit() else 99):
                if txt_sents[k]:
                    transcripts_items.append(txt_sents[k])
            if txt_para:
                transcripts_items.append(txt_para)
        except Exception:
            pass

        dlg = QDialog(self)
        dlg.setWindowTitle("Auto-Detect — pick label vocabulary")
        dlg.setMinimumWidth(640)
        lay = QVBoxLayout(dlg)
        lay.addWidget(QLabel(
            "<b>Snap each detected chunk's prediction to the closest known label.</b><br>"
            "Picks below provide the vocabulary. Free-form keeps the model's raw output."
        ))

        # Option 1: wordlist (chars + words + sentences + paragraph)
        rb_wordlist = QRadioButton(
            f"Use loaded wordlist  "
        ...
        # Option 2: transcripts/<iso>.txt
        rb_transcripts = QRadioButton(
            f"Use transcripts/{iso}.txt  ({len(transcripts_items)} items)"
        )
        rb_transcripts.setEnabled(bool(transcripts_items))
        lay.addWidget(rb_transcripts)

        # Option 3: custom paste
        rb_paste = QRadioButton("Paste a custom list (one label per line)")
        lay.addWidget(rb_paste)
        txt_paste = QPlainTextEdit()
        txt_paste.setPlaceholderText("one label per line, e.g.:\n  अ\n  आ\n  apple\n")
        txt_paste.setMinimumHeight(110)
        lay.addWidget(txt_paste)

        # Option 4: free-form (legacy behavior — useful for unscripted audio)
        rb_free = QRadioButton(
            "Free-form (no snapping) — use the model's raw transcription as the label"
        )
        lay.addWidget(rb_free)

        # If we have nothing else, default to free-form so the dialog isn't useless
        if not wordlist_seq and not transcripts_items:
            rb_free.setChecked(True)

        group = QButtonGroup(dlg)
        for rb in (rb_wordlist, rb_transcripts, rb_paste, rb_free):
            group.addButton(rb)
        ...
        if rb_wordlist.isChecked():
            return wordlist_seq
        if rb_transcripts.isChecked():
            return transcripts_items
        if rb_paste.isChecked():
    ...
    def on_auto_detect(self):
        # Auto-Detect now always asks for a vocab source. If the user picks
        # free-form, the thread behaves exactly as before (legacy mode).
        if self.y is None:
            QMessageBox.warning(self, "", "Load an audio file first.")
            return
        vocab = self._ask_autodetect_vocab()
        if vocab is None:
            return  # cancelled
        self._run_auto_detect(do_asr=True, vocab=vocab)

    def on_vad_only(self):
        # VAD-only never transcribes, so vocab snapping has nothing to act on
        self._run_auto_detect(do_asr=False, vocab=None)

    def _run_auto_detect(self, do_asr, vocab=None):
        if self.y is None:
            QMessageBox.warning(self, "", "Load an audio file first.")
            return
        if do_asr:
        ...
        # Reset merge-warning counter for this run
        self._autodetect_merge_warnings = []

        iso, name = self._current_lang()
        t = AutoSegmentThread(
            self.y, self.sr, self.y16, iso, name, self._vad_params(),
            do_asr=do_asr, vocab=vocab,
        )
        ...
        if do_asr and vocab:
            mode_str = f"VAD + ASR + snap-to-vocab ({len(vocab)} items)"
        elif do_asr:
            mode_str = "VAD + ASR (free-form labels)"
        else:
            mode_str = "VAD only"
    ...
    def _set_auto_busy(self, busy):
        ...
        for b in (self.btn_auto_detect, self.btn_vad_only, self.btn_align_gt,
                  self.btn_predict_gt, self.btn_detect_splits,
    ...
    def _on_auto_segment(self, seg):
        ...
        # Collect merge-warning hints for the end-of-run summary
        if seg.get("_merge_warning"):
            self._autodetect_merge_warnings.append({
                "idx": len(self.segments),
                "label": seg.get("label", ""),
                "start": seg.get("start", 0),
                "end": seg.get("end", 0),
                "reason": seg["_merge_warning"],
    ...
    def _on_auto_done(self, total, elapsed):
        ...
        warnings = getattr(self, "_autodetect_merge_warnings", [])
        # Snap stats: how many segments had snap_score set, and the breakdown
        snapped = [s for s in self.segments if s.get("_snap_score") is not None]
        good_snaps = sum(1 for s in snapped if s["_snap_score"] >= 0.85)
        partial_snaps = sum(1 for s in snapped if 0.55 <= s["_snap_score"] < 0.85)
        no_match = sum(1 for s in snapped if s["_snap_score"] < 0.55)

        parts = [f"✓ Auto-detect finished: {total} segments in "
                 f"{elapsed:.1f}s ({speed:.1f}/s)"]
        if snapped:
            parts.append(
                f"snap quality: {good_snaps} confident · "
                f"{partial_snaps} fuzzy · {no_match} kept raw"
            )
        if warnings:
            parts.append(f"⚠ {len(warnings)} possible chunk-merges (see popup)")
        parts.append("review and Save")
        self._set_status("  │  ".join(parts))

        # If we detected likely chunk-merges, surface them — this is the
        # safeguard against silently losing labels when VAD lumps several
        # rapid alphabets into one chunk.
        if warnings:
            preview_lines = []
            for w in warnings[:15]:
                preview_lines.append(
                    f"  • Segment {w['idx']:3d}  "
                    f"[{format_hms(w['start'])} → {format_hms(w['end'])}]  "
                    f"label='{w['label']}'\n"
                    f"      prediction: '{w['prediction']}'\n"
                    f"      reason: {w['reason']}"
                )
            more = f"\n\n  … (+{len(warnings) - 15} more)" if len(warnings) > 15 else ""
            QMessageBox.warning(
                self, "Possible chunk-merges detected",
                f"{len(warnings)} segment(s) look like VAD may have merged "
                f"multiple expected labels into a single chunk. The label "
                f"shown is only the *first* matching vocab entry — adjacent "
                f"labels may be missing from your output.\n\n"
                + "\n".join(preview_lines) + more
                + "\n\nFix options:\n"
                "  • Tune VAD: lower silence_ms (e.g. 150 ms) or raise threshold "
                "(e.g. -35 dB) so each item becomes its own chunk, then re-run.\n"
                "  • Split each flagged segment manually (mark a sub-region and "
                "Add Segment for each missing label)."
            )
    ...
    # ----- Split detection: one reference text -> multiple chunks -----
    def on_detect_splits(self):
        """Look for cases where the speaker split one reference paragraph
        (or one sentence) across multiple audio segments, and offer to
        relabel them p1/p2/p3 (or s2a/s2b) in reading order."""
        if not self.segments:
            QMessageBox.warning(self, "", "No segments to analyze.")
            return
        # Need at least some segments with predictions, otherwise we can't
        # match them against the reference text.
        n_with_pred = sum(1 for s in self.segments if (s.get("prediction") or "").strip())
        if n_with_pred == 0:
            QMessageBox.warning(
                self, "",
                "No segments have a model prediction yet.\n\n"
                "Run 🎯 Predict on GT first so each segment carries the "
                "model's transcription — that's how splits are detected."
            )
            return

        # Run both detectors using whatever reference texts are loaded
        para_groups = detect_paragraph_splits(
            self.segments, self.current_paragraph,
        )
        sent_groups = detect_sentence_splits(
            self.segments, self.current_sentences,
        )

        if not para_groups and not sent_groups:
            self._set_status(
                "🧩 No splits detected — segments appear to be one-per-reference."
            )
            QMessageBox.information(
                self, "No splits detected",
                "Each paragraph/sentence reference appears to fit in a single "
                "audio segment.\n\n"
                "If you believe a split was missed, check that:\n"
                "  • Run 🎯 Predict on GT first to get per-segment predictions\n"
                "  • The reference paragraph (from docx/transcripts) is loaded\n"
                "  • Adjacent paragraph segments have a time-gap under 20s"
            )
            return
        ...
        lines = []
        if para_groups:
            lines.append(f"<b>Paragraph splits: {len(para_groups)}</b>")
            for g in para_groups:
                ref_n = g["ref_word_count"]
                covered = sum(e - s + 1 for s, e in g["spans"])
                lines.append(
                    f"&nbsp;&nbsp;• Segments {[i+1 for i in g['indices']]} → "
                    f"{', '.join(g['new_labels'])}  "
                    f"<span style='color:#666'>(covers ~{covered}/{ref_n} words)</span>"
                )
                for idx, (s, e), new_lbl in zip(g["indices"], g["spans"], g["new_labels"]):
                    seg = self.segments[idx]
                    ref_words = self.current_paragraph.split()
                    span_text = " ".join(ref_words[s:e + 1])
                    if len(span_text) > 60:
                        span_text = span_text[:57] + "…"
                    lines.append(
                        f"&nbsp;&nbsp;&nbsp;&nbsp;<code>{new_lbl}</code> "
                        f"[{format_hms(seg['start'])}–{format_hms(seg['end'])}]  "
                        f"<span style='color:#0f766e'>“{span_text}”</span>"
                    )
        if sent_groups:
            if lines: lines.append("")
            lines.append(f"<b>Sentence splits: {len(sent_groups)}</b>")
            for g in sent_groups:
                lines.append(
                    f"&nbsp;&nbsp;• Segments {[i+1 for i in g['indices']]} "
                    f"(currently all <code>{g['sentence_id']}</code>) → "
                    f"{', '.join(g['new_labels'])}"
                )

        # Use a richer dialog than QMessageBox so we can show formatted text
        dlg = QDialog(self)
        dlg.setWindowTitle("Detected splits — apply relabel?")
        dlg.setMinimumWidth(640)
        v = QVBoxLayout(dlg)
        intro = QLabel(
            "<b>The tool detected segments that together cover a single "
            "reference text.</b> Relabel them in reading order?"
        )
        ...
        v.addWidget(scroll)
        bb = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        bb.button(QDialogButtonBox.Ok).setText("Apply relabel")
        bb.accepted.connect(dlg.accept)
        bb.rejected.connect(dlg.reject)
        v.addWidget(bb)
        if dlg.exec_() != QDialog.Accepted:
            self._set_status("🧩 Split relabel cancelled.")
            return
        ...
        for g in para_groups:
            ref_words = self.current_paragraph.split()
            for idx, (ws, we), new_label in zip(g["indices"], g["spans"], g["new_labels"]):
                seg = self.segments[idx]
                seg["label"] = new_label
                # Stash the actual text span this chunk covered — useful
                # downstream for the JSONL manifest and for reviewers.
                seg["_split_ref_text"] = " ".join(ref_words[ws:we + 1])
                seg["_split_ref_span_words"] = [ws, we]
                # Clearing the partial-paragraph fraction since the chunk
                # is no longer "partial" — it now legitimately *is* p1/p2/p3.
                seg.pop("_para_fraction", None)
                seg.pop("_para_spoken_words", None)
                seg.pop("_para_total_words", None)
                n_applied += 1
        for g in sent_groups:
            for idx, new_label in zip(g["indices"], g["new_labels"]):
                seg = self.segments[idx]
                seg["label"] = new_label
                seg["_split_ref_text"] = g["ref_text"]
                n_applied += 1
        ...
        if self.y is not None:
            self._redraw(xlim=self.ax.get_xlim())
        self._set_status(
            f"🧩 Relabeled {n_applied} segments across "
            f"{len(para_groups)} paragraph split(s) and "
            f"{len(sent_groups)} sentence split(s). Ctrl+Z to undo."
        )
    ...
    def _refresh_seg_list(self):
        ...
        for i, seg in enumerate(self.segments):
            ...
            merge_warn = seg.get("_merge_warning")
            # Marker priority:
            #   ⚠ = Auto-Detect flagged a possible chunk-merge (highest priority)
            #   ✓ = label & prediction agree exactly
            #   ✎ = human edited a non-matching prediction
            ...
            # Coloring + tooltip — merge-warning takes priority
            if merge_warn:
                item.setForeground(QColor("#b91c1c"))  # red
                item.setToolTip(
                    f"⚠ Possible chunk-merge.\n\n"
                    f"PRED (model):     {pred}\n"
                    f"Snapped label:    {label}\n\n"
                    f"Reason: {merge_warn}\n\n"
                    f"VAD likely merged multiple expected labels into this "
                    f"one chunk. Adjacent labels may be missing from your "
                    f"output. Consider splitting this segment manually or "
                    f"tuning VAD (lower silence_ms, raise threshold) and "
                    f"re-running Auto-Detect."
                )
            elif (frac := seg.get("_para_fraction")) is not None and frac < 0.95:
    ...
    def _update_score_panel(self):
        ...
        html = (
            ...
            f"<b>WER:</b> {m['avg_wer']:.3f}  ·  "
            f"<b>CER:</b> {m['avg_cer']:.3f}<br>"
            f"<b>Boundary IoU:</b> "
            + (f"median {m['median_iou']:.3f}  ·  "
               f"≥0.5: {m['iou_above_0_5']*100:.0f}%  ·  "
               f"≥0.8: {m['iou_above_0_8']*100:.0f}%  "
               f"<span style='color:#888'>(n={m['n_iou']})</span>"
               if m['n_iou'] else "<i>run 🎯 Predict on GT</i>")
            + "<br>"
        ...
        # --- Per-task-type breakdown table (HTML) ---
        per_type = m.get("per_type_metrics") or {}
        if per_type:
            order = ["char", "word", "sentence", "paragraph"]
            present = [t for t in order if t in per_type] + \
                      [t for t in per_type if t not in order]
            html += (
                "<div style='margin:4px 0 6px;font-weight:bold;font-size:11px'>"
                "Per-task-type breakdown:</div>"
                "<table cellspacing='0' cellpadding='3' "
                "style='border-collapse:collapse;font-size:10.5px;margin-bottom:8px'>"
                "<tr style='background:#e2e8f0;font-weight:bold'>"
                "<td>type</td><td>n</td><td>pred</td><td>accept</td>"
                "<td>CER</td><td>WER</td><td>mIoU</td>"
                "<td>IoU≥.5</td><td>time/s</td></tr>"
            )
            def _cell(v, fmt, ok):
                return f"<td style='text-align:right'>{fmt.format(v) if ok else '—'}</td>"
            for t in present:
                s = per_type[t]
                has_pred = s['n_with_pred'] > 0
                has_iou = s['n_iou'] > 0
                has_time = s['n_timed'] > 0
                row_html = (
                    f"<tr><td><b>{t}</b></td>"
                    f"<td style='text-align:right'>{s['n']}</td>"
                    f"<td style='text-align:right'>{s['n_with_pred']}</td>"
                    + _cell(s['acceptance_rate']*100, "{:.0f}%", has_pred)
                    + _cell(s['avg_cer'], "{:.3f}", has_pred)
                    + _cell(s['avg_wer'], "{:.3f}", has_pred)
                    + _cell(s['median_iou'], "{:.3f}", has_iou)
                    + _cell(s['iou_above_0_5']*100, "{:.0f}%", has_iou)
                    + _cell(s['mean_time_s'], "{:.1f}", has_time)
                    + "</tr>"
                )
                html += row_html
            html += "</table>"
    ...
    # ----- Paper-grade metrics -----
    @staticmethod
    def _edit_distance(a_seq, b_seq):
        """Generic Levenshtein over any sequence of comparable tokens.
        Used by both WER (token = word) and CER (token = character)."""
        ...
        for i in range(1, n + 1):
            ...
            for j in range(1, m + 1):
                tmp = dp[j]
                cost = 0 if a_seq[i - 1] == b_seq[j - 1] else 1
                dp[j] = min(prev + cost, dp[j] + 1, dp[j - 1] + 1)
                prev = tmp
        return dp[m]

    @classmethod
    def _word_error_rate(cls, ref, hyp):
        """Standard WER. Tokenizes on whitespace.
        Returns edits / len(ref_words). Returns 0 if both empty, 1 if only
        ref is empty (a non-empty hyp against empty ref is fully wrong)."""
        r = (ref or "").split()
        h = (hyp or "").split()
        if not r:
            return 0.0 if not h else 1.0
        return cls._edit_distance(r, h) / len(r)

    @classmethod
    def _character_error_rate(cls, ref, hyp):
        """CER = edit distance on CHARACTERS / len(ref_chars).

        Whitespace is stripped before comparison so 'a b c' vs 'abc'
        scores as identical (this matters for char/word labels where
        ASR sometimes inserts spaces between characters).

        CER is more honest than WER for:
          - Single-character labels (where one word-level error = 100% WER)
          - Indic scripts where word boundaries are ambiguous
          - Comparing models that tokenize differently
        """
        r = re.sub(r"\s+", "", ref or "")
        h = re.sub(r"\s+", "", hyp or "")
        if not r:
    ...
    @staticmethod
    def _segment_iou(gt_start, gt_end, pred_start, pred_end):
        """Temporal Intersection-over-Union for two time intervals.

        Returns a value in [0, 1]:
          1.0 = identical intervals
          0.0 = no overlap at all
          0.5 = standard 'usable' threshold in segmentation literature

        Normalises by segment length, so a 0.1s error on a 0.5s alphabet
        utterance (IoU ~0.67) and the same error on a 5s paragraph
        (IoU ~0.98) get appropriately different scores.
        """
        if pred_start is None or pred_end is None:
            return None
        if gt_end <= gt_start or pred_end <= pred_start:
            return None
        overlap = max(0.0, min(gt_end, pred_end) - max(gt_start, pred_start))
    ...
    def compute_metrics(self):
        """Compute the full metric bundle for the on-screen report,
        the exported paper report, and the analytics dashboard.

        Returns a dict with both global aggregates AND a per-task-type
        breakdown ('char' / 'word' / 'sentence' / 'paragraph'), so the
        paper can show "ASR is great on sentences, weak on single chars"
        which is the real story for read-aloud Indic data.
        """
        iso, lang_name = self._current_lang()
        self._timing_flush()  # make sure the active segment's time is counted

        # Per-type accumulators — every metric tracked once globally and
        # once per task type. The empty-bucket structure makes the rest of
        # the code uniform: just keep appending into the right bucket.
        def _new_bucket():
            return {
                "n": 0,
                "n_with_pred": 0,
        ...
        for seg in self.segments:
            ...
            pred = (seg.get("prediction") or "").strip()
            stype = classify_segment_type(label)
            tb = by_type.setdefault(stype, _new_bucket())

            # For sequence-ID labels like 's1' or 'paragraph', WER/CER against
            # the literal label is meaningless ('s1' vs 'the cat sat...' is
            # 600% WER, which tells us nothing). Use the reference TEXT from
            # the loaded wordlist/transcripts instead. This is what reviewers
            # actually want: text-quality between pred and the expected speech.
            ref_text_for_metrics = label
            m_s = _RE_SEQ_SENT.match(label)
            m_p = _RE_SEQ_PARA.match(label)
            if m_s and label in self.current_sentences:
                ref_text_for_metrics = self.current_sentences[label]
            elif (m_p or stype == "paragraph") and self.current_paragraph:
                ref_text_for_metrics = self.current_paragraph

            # Counts
            global_b["n"] += 1; tb["n"] += 1
            dur = float(seg.get("end", 0)) - float(seg.get("start", 0))
            global_b["durations"].append(dur); tb["durations"].append(dur)

            # Timing log
            uid = self._seg_uid(seg)
            if uid in self._timing_log:
                v = self._timing_log[uid]
                global_b["timing_vals"].append(v)
                tb["timing_vals"].append(v)

            # Text-quality metrics — only when we have a prediction to score.
            # `ref_text_for_metrics` is the literal label for chars/words, but
            # the resolved reference text for s1..s5 / paragraph segments.
            if pred:
                global_b["n_with_pred"] += 1; tb["n_with_pred"] += 1
                sim = _fuzzy_score(pred, ref_text_for_metrics)
                wer = self._word_error_rate(ref_text_for_metrics, pred)
                cer = self._character_error_rate(ref_text_for_metrics, pred)
                global_b["sims"].append(sim); tb["sims"].append(sim)
                global_b["wers"].append(wer); tb["wers"].append(wer)
                global_b["cers"].append(cer); tb["cers"].append(cer)
                # Acceptance compares the LITERAL label (the annotator's text)
                # to the prediction — that's the right "kept verbatim" check.
                if _norm_for_compare(pred) == _norm_for_compare(label):
                    global_b["accepted"] += 1; tb["accepted"] += 1
                else:
                    global_b["edited"] += 1; tb["edited"] += 1

            # Timing-quality metrics — only when we have predicted timestamps
            ps = seg.get("pred_start"); pe = seg.get("pred_end")
            se = seg.get("start_err"); ee = seg.get("end_err")
            if se is not None and ee is not None:
                global_b["start_errs"].append(se); tb["start_errs"].append(se)
                global_b["end_errs"].append(ee); tb["end_errs"].append(ee)
                global_b["abs_start_errs"].append(abs(se))
                tb["abs_start_errs"].append(abs(se))
                global_b["abs_end_errs"].append(abs(ee))
                tb["abs_end_errs"].append(abs(ee))
            iou = self._segment_iou(
                float(seg.get("start", 0)), float(seg.get("end", 0)),
                ps, pe,
            )
            if iou is not None:
                global_b["ious"].append(iou); tb["ious"].append(iou)

            # Partial-paragraph counter (global only — only paragraphs have it)
            frac = seg.get("_para_fraction")
            if frac is not None and frac < 0.95:
                partial_paras += 1

        # Helpers for summary stats over a bucket
        def _mean(xs): return sum(xs) / len(xs) if xs else 0.0
        def _median(xs):
            if not xs: return 0.0
            s = sorted(xs); n = len(s)
            return s[n // 2] if n % 2 else (s[n // 2 - 1] + s[n // 2]) / 2
        def _frac_above(xs, thr):
            return (sum(1 for x in xs if x >= thr) / len(xs)) if xs else 0.0

        def _summarize(b):
            """Roll up a bucket into the reportable stats."""
            n_pred = b["n_with_pred"]
            return {
                "n": b["n"],
                "n_with_pred": n_pred,
                "accepted": b["accepted"],
                "edited": b["edited"],
                "acceptance_rate": (b["accepted"] / n_pred) if n_pred else 0.0,
                "avg_similarity": _mean(b["sims"]),
                "avg_wer": _mean(b["wers"]),
                "avg_cer": _mean(b["cers"]),
                "median_iou": _median(b["ious"]),
                "mean_iou": _mean(b["ious"]),
                "iou_above_0_5": _frac_above(b["ious"], 0.5),
                "iou_above_0_8": _frac_above(b["ious"], 0.8),
                "n_iou": len(b["ious"]),
                "mae_start": _mean(b["abs_start_errs"]),
                "mae_end": _mean(b["abs_end_errs"]),
                "mean_start_err": _mean(b["start_errs"]),
                "mean_end_err": _mean(b["end_errs"]),
                "n_timed": len(b["timing_vals"]),
                "mean_time_s": _mean(b["timing_vals"]),
                "median_time_s": _median(b["timing_vals"]),
                "total_time_s": sum(b["timing_vals"]),
                "total_dur_s": sum(b["durations"]),
            }

        global_summary = _summarize(global_b)
        per_type_summary = {t: _summarize(b) for t, b in by_type.items()}

        # Flatten global stats into the top-level dict so existing callers
        # that read m["acceptance_rate"], m["mae_start"], etc. still work.
        result = dict(global_summary)
        result.update({
            "language": lang_name,
            "iso": iso,
            ...
            "per_type": {t: b["n"] for t, b in by_type.items()},  # backcompat: counts only
            "per_type_metrics": per_type_summary,                  # NEW: full metrics
            "partial_paragraphs": partial_paras,
            "total_labeled_dur_s": global_summary["total_dur_s"],
        })
        return result
    ...
    def _format_metrics_report(self, m):
        ...
        Layout:
          - Header (audio, language, totals)
          - Segment-type counts
          - Global ASR text-quality (acceptance, CER, WER, similarity)
          - Global timing quality (MAE + IoU)
          - Annotation timing
          - Per-task-type breakdown table (the table to paste into the paper)
        """
        lines = []
        lines.append("=" * 72)
        ...
        lines.append("")
        lines.append("-- ASR pre-labeling quality (global) --")
        ...
        lines.append(f"   avg WER (word err rate)  : {m['avg_wer']:.3f}")
        lines.append(f"   avg CER (char err rate)  : {m['avg_cer']:.3f}")
        lines.append("")
        lines.append("-- Timestamp prediction quality (pred vs GT) --")
        if m["mae_start"] or m["mae_end"] or m["n_iou"]:
            ...
            if m["n_iou"]:
                lines.append(f"   boundary IoU       : median {m['median_iou']:.3f}  "
                             f"mean {m['mean_iou']:.3f}  (over {m['n_iou']} segs)")
                lines.append(f"     IoU ≥ 0.50       : "
                             f"{m['iou_above_0_5']*100:.1f}%   "
                             f"(threshold of 'usable' segmentation)")
                lines.append(f"     IoU ≥ 0.80       : "
                             f"{m['iou_above_0_8']*100:.1f}%   "
                             f"(threshold of 'tight' segmentation)")
        else:
        ...
        # This is the table to paste into a paper. Shows how ASR + the human
        # workflow perform separately on each granularity (char/word/sent/para),
        # which is where the real story is: e.g., "ASR is great on sentences,
        # weak on isolated chars" is much more informative than one average.
        per_type = m.get("per_type_metrics") or {}
        if per_type:
            order = ["char", "word", "sentence", "paragraph"]
            present = [t for t in order if t in per_type] + \
            ...
            header = (f"   {'type':<10} {'n':>4} {'pred':>5} {'accept':>7} "
                      f"{'CER':>6} {'WER':>6} {'mIoU':>6} "
                      f"{'IoU≥.5':>7} {'time/s':>7}")
            sep = "   " + "-" * (len(header) - 3)
            lines.append(header)
            lines.append(sep)
            for t in present:
                s = per_type[t]
                cer = f"{s['avg_cer']:.3f}" if s['n_with_pred'] else "  —  "
                wer = f"{s['avg_wer']:.3f}" if s['n_with_pred'] else "  —  "
                iou = f"{s['median_iou']:.3f}" if s['n_iou'] else "  —  "
                iou5 = f"{s['iou_above_0_5']*100:5.1f}%" if s['n_iou'] else "  —   "
                tps = f"{s['mean_time_s']:.1f}" if s['n_timed'] else "  —  "
                acc = (f"{s['acceptance_rate']*100:5.1f}%"
                       if s['n_with_pred'] else "  —   ")
                lines.append(
                    f"   {t:<10} {s['n']:>4} {s['n_with_pred']:>5} "
                    f"{acc:>7} {cer:>6} {wer:>6} {iou:>6} {iou5:>7} {tps:>7}"
                )
            lines.append("")
            lines.append("   Legend:  CER = character error rate (lower = better)")
            lines.append("            WER = word error rate (lower = better)")
            lines.append("            mIoU = median temporal IoU of pred vs GT bounds")
            lines.append("            IoU≥.5 = % of segments meeting 'usable' threshold")
        lines.append("=" * 72)
        return "\n".join(lines)
