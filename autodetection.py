...
# ---------- Level-wise validation helpers (char / word / sentence / paragraph) ----------
_SENT_SPLIT_RE = re.compile(r"(?<=[.!?।])\s+|\n+", re.UNICODE)
_WORD_RE = re.compile(rf"[A-Za-z0-9{_INDIC_RE}]+(?:[-'][A-Za-z0-9{_INDIC_RE}]+)?", re.UNICODE)


def _norm_for_match(text):
    """Normalize text for comparison without destroying Indic characters."""
    if text is None:
        return ""
...
def _text_similarity(a, b):
    """Similarity in [0, 1] for validation reports."""
    aa = _norm_for_match(a)
    bb = _norm_for_match(b)
...
def _split_words_with_spans(text):
    """Return [(word, char_start, char_end), ...] over the original string."""
    if not text:
        return []
    return [(m.group(0), m.start(), m.end()) for m in _WORD_RE.finditer(str(text))]


def _split_chars_with_spans(text):
    """Return visible non-space chars with spans. Combining marks remain attached when possible."""
    if not text:
        return []
    units = []
    s = str(text)
    ...
    while i < len(s):
        ...
        if ch.isspace():
            ...
            continue
        j = i + 1
        # Keep Indic combining signs with their base character.
        while j < len(s) and ("\u0900" <= s[j] <= "\u097F" or "\u0A80" <= s[j] <= "\u0AFF") and unicodedata.category(s[j]).startswith("M"):
            j += 1
        units.append((s[i:j], i, j))
        i = j
    return units


def _split_sentences_with_spans(text):
    """Return sentence-like chunks with spans over original string."""
    if not text:
        return []
    s = str(text)
    out = []
    ...
    for m in _SENT_SPLIT_RE.finditer(s):
        ...
        if chunk:
            # Trim span to non-space bounds
            l = start + len(s[start:end]) - len(s[start:end].lstrip())
            r = end - (len(s[start:end]) - len(s[start:end].rstrip()))
            out.append((s[l:r], l, r))
        start = m.end()
    chunk = s[start:].strip()
    if chunk:
        l = start + len(s[start:]) - len(s[start:].lstrip())
        r = len(s) - (len(s[start:]) - len(s[start:].rstrip()))
        out.append((s[l:r], l, r))
    return out


def _span_to_time(seg_start, seg_end, char_start, char_end, total_chars):
    """Approximate unit timestamp inside a predicted/GT segment from text char spans."""
    dur = max(0.0, float(seg_end) - float(seg_start))
    total = max(1, int(total_chars))
    st = float(seg_start) + dur * (max(0, char_start) / total)
    en = float(seg_start) + dur * (max(char_start + 1, char_end) / total)
    return st, min(float(seg_end), en)


def _make_unit(level, idx, text, start, end, source_seg=None):
    return {
        "level": level,
        "idx": idx,
        ...
        "start": float(start),
        "end": float(end),
        "source_label": (source_seg or {}).get("label", ""),
        "source_prediction": (source_seg or {}).get("prediction", ""),
    }


def _units_from_text_segment(text, start, end, levels=("char", "word", "sentence", "paragraph"), source_seg=None):
    """Split one segment's text into requested levels and approximate sub-unit timestamps."""
    text = "" if text is None else str(text)
    total = max(1, len(text))
    out = {lvl: [] for lvl in levels}

    if "char" in out:
        for ch, a, b in _split_chars_with_spans(text):
            st, en = _span_to_time(start, end, a, b, total)
            out["char"].append(_make_unit("char", len(out["char"]) + 1, ch, st, en, source_seg))

    if "word" in out:
        for wd, a, b in _split_words_with_spans(text):
            st, en = _span_to_time(start, end, a, b, total)
            out["word"].append(_make_unit("word", len(out["word"]) + 1, wd, st, en, source_seg))

    if "sentence" in out:
        for sent, a, b in _split_sentences_with_spans(text):
            st, en = _span_to_time(start, end, a, b, total)
            out["sentence"].append(_make_unit("sentence", len(out["sentence"]) + 1, sent, st, en, source_seg))

    if "paragraph" in out and text.strip():
        out["paragraph"].append(_make_unit("paragraph", 1, text, start, end, source_seg))

    return out


def _infer_gt_level(label):
    """Infer level from a GT label row."""
    lab = (label or "").strip()
    low = lab.lower()
    if low == "paragraph" or low.startswith("paragraph"):
        return "paragraph"
    if re.fullmatch(r"s\d+", low):
        return "sentence"
    if len(lab) <= 4 and len(_split_words_with_spans(lab)) <= 1:
        # Covers English single letters and Indic grapheme-ish labels.
        return "char"
    return "word"


def _load_annotation_segments(path):
    """Load label/start/end[/prediction] rows from csv/tsv/xlsx; returns list of segments."""
    rows = _read_table(path)
    if rows:
        first = [str(c).strip().lower() for c in rows[0]]
        if (len(first) >= 3 and
            ("label" in first[0] or "gt" in first[0]) and
...
def _gt_units_from_segments(gt_segments, sentences=None, paragraph=""):
    """Build GT units from label/start/end rows. s1..sN and paragraph labels use transcript text if available."""
    sentences = sentences or {}
    units = {"char": [], "word": [], "sentence": [], "paragraph": []}
    for seg in gt_segments:
        lab = (seg.get("label") or "").strip()
        level = _infer_gt_level(lab)
        low = lab.lower()
        text = lab
        if level == "sentence" and low in sentences:
            text = sentences[low]
        elif level == "paragraph" and paragraph:
            text = paragraph
        units[level].append(_make_unit(level, len(units[level]) + 1, text, seg["start"], seg["end"], seg))

        # If the GT row is a sentence or paragraph, also create lower-level GT units by interpolation.
        # This lets a sentence-level GT file still validate char/word timing approximately.
        if level in ("sentence", "paragraph") and text:
            expanded = _units_from_text_segment(text, seg["start"], seg["end"],
                                                levels=("char", "word", "sentence"), source_seg=seg)
            for lower in ("char", "word"):
                units[lower].extend(expanded[lower])
            if level == "paragraph":
                units["sentence"].extend(expanded["sentence"])

    # Re-number after merging expanded units
    for lvl in units:
        for i, u in enumerate(units[lvl], start=1):
            u["idx"] = i
    return units


def _pred_units_from_segments(pred_segments):
    """Build prediction units from current segments. Uses `prediction` when present, otherwise label text."""
    units = {"char": [], "word": [], "sentence": [], "paragraph": []}
    para_text = []
    para_start = None
    para_end = None
    for seg in pred_segments:
        txt = (seg.get("prediction") or seg.get("label") or "").strip()
        if not txt:
            continue
        expanded = _units_from_text_segment(txt, seg["start"], seg["end"],
                                            levels=("char", "word", "sentence"), source_seg=seg)
        for lvl in ("char", "word", "sentence"):
            units[lvl].extend(expanded[lvl])
        para_text.append(txt)
        para_start = seg["start"] if para_start is None else min(para_start, seg["start"])
        para_end = seg["end"] if para_end is None else max(para_end, seg["end"])

    if para_text and para_start is not None and para_end is not None:
        units["paragraph"].append(_make_unit("paragraph", 1, " ".join(para_text), para_start, para_end, {}))
...
def _compare_units(gt_units, pred_units):
    """Return report rows with signed timestamp error: pred - GT."""
    rows = []
    for lvl in ("char", "word", "sentence", "paragraph"):
        g = gt_units.get(lvl, [])
        p = pred_units.get(lvl, [])
        n = max(len(g), len(p))
        for i in range(n):
            gu = g[i] if i < len(g) else None
            pu = p[i] if i < len(p) else None
            if gu and pu:
                sim = _text_similarity(gu["text"], pu["text"])
                start_err = pu["start"] - gu["start"]
                end_err = pu["end"] - gu["end"]
                dur_err = (pu["end"] - pu["start"]) - (gu["end"] - gu["start"])
                status = "OK" if sim >= 0.80 and abs(start_err) <= 0.25 and abs(end_err) <= 0.25 else "CHECK"
                note = "+ve time error = prediction timestamp is later/longer than GT; -ve = earlier/shorter."
                if lvl == "paragraph":
                    gt_words = len(_split_words_with_spans(gu["text"]))
                    pr_words = len(_split_words_with_spans(pu["text"]))
                    coverage = pr_words / max(1, gt_words)
                    if coverage < 0.80:
                        status = "PARTIAL_PARAGRAPH"
                        note = f"Paragraph looks only partly spoken/transcribed: {pr_words}/{gt_words} words ({coverage:.0%})."
                rows.append([
                    lvl, i + 1,
                    gu["text"], pu["text"],
                    format_hms(gu["start"]), format_hms(gu["end"]),
                    format_hms(pu["start"]), format_hms(pu["end"]),
                    round(start_err, 3), round(end_err, 3), round(dur_err, 3),
                    round(sim, 3), status, note,
                ])
            elif gu and not pu:
                rows.append([
                    lvl, i + 1, gu["text"], "",
                    format_hms(gu["start"]), format_hms(gu["end"]),
                    "", "", "", "", "", 0.0, "MISSING_PRED",
                    "GT has this unit but model did not produce a corresponding prediction.",
                ])
            elif pu and not gu:
                rows.append([
                    lvl, i + 1, "", pu["text"],
                    "", "", format_hms(pu["start"]), format_hms(pu["end"]),
                    "", "", "", 0.0, "EXTRA_PRED",
                    "Model produced an extra unit that has no GT counterpart.",
                ])
    return rows
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.btn_validate_levels = QPushButton("📊 Validate Levels")
        self.btn_validate_levels.setStyleSheet(
            "QPushButton{font-weight:bold;color:white;background:#dc2626;"
            "border:1px solid #b91c1c;padding:5px 12px;border-radius:4px}"
            "QPushButton:hover{background:#b91c1c}"
            "QPushButton:disabled{color:#9ca3af;background:#f3f4f6}"
        )
        self.btn_validate_levels.setToolTip(
            "Compare current predictions against a GT file at four levels:\n"
            "char-by-char, word-by-word, sentence-by-sentence and paragraph-by-paragraph.\n\n"
            "Output columns include GT start/end, predicted start/end, signed timestamp error "
            "(prediction minus GT), text similarity, and PARTIAL_PARAGRAPH when only part of "
            "the paragraph was spoken/transcribed."
        )
        self.btn_validate_levels.clicked.connect(self.on_validate_levels)
        r2.addWidget(self.btn_validate_levels)
        ...
        # Set a sensible minimum width on the right panel so its labels and
        # buttons never clip when the splitter is dragged narrow. Put it
        # directly into the splitter (the old QScrollArea wrap was clipping
        # the labels on their left side).
        right_tabs.setMinimumWidth(540)

        splitter.addWidget(right_tabs)
        # Give the waveform ~65% by default but let the right panel grow
        splitter.setStretchFactor(0, 3)
        splitter.setStretchFactor(1, 2)
        splitter.setSizes([1150, 600])
    ...
    def _on_lang_changed(self):
        ...
        # ALWAYS try transcripts/<iso>.txt as the authoritative source of
        # sentences + paragraph. This file is shared with app.py and is the
        # canonical reference text — if present, override whatever we got
        # from the docx (which sometimes mis-parses bulleted sentences).
        try:
            ...
            if txt_sentences or txt_paragraph:
                log(f"Loaded transcripts/{iso}.txt: "
    ...
    def _redraw(self, xlim=None):
        ...
        pred_label_y = y_bot + y_span * 0.12          # PRED (model) badge — bottom
        ...
        for i, seg in enumerate(self.segments):
            ...
            # (cheap glanceable signal even when badges are hidden)
            if differs:
                self.ax.plot([(s + e) / 2], [marker_y + y_span * 0.02],
                             marker="v", markersize=5, color=DIFF_INDICATOR,
            ...
            # Only show if a prediction exists; if it matches GT exactly, show
            # a compact "✓ matches GT" hint instead of duplicating the text.
            if pred:
                pred_text = (
                    f"PRED  {_trunc(pred)}"
    ...
    def _refresh_seg_list(self):
        ...
        for i, seg in enumerate(self.segments):
            ...
            label_disp = label if len(label) <= 22 else label[:21] + "…"
            txt = (f"{i+1:03d} {marker} {label_disp:<22}  "
                   f"{format_hms(seg['start'])} → {format_hms(seg['end'])}  "
                   f"(dur {seg['end']-seg['start']:.3f}s)")
            item = QListWidgetItem(txt)
            # Color the row when there's something to flag
            if marker == "✎":
                item.setForeground(QColor("#b45309"))  # amber: prediction differs
                tt = (f"GT (your label):  {label}\n"
                      f"PRED (model):     {pred}\n\n"
                      f"Marker '✎' = your label differs from the model's prediction.")
                item.setToolTip(tt)
            elif marker == "✓":
                ...
                item.setToolTip(f"Label: {label}\n\nNo prediction (manual segment).")
    ...
    def on_validate_levels(self):
        """Level-wise validation against a GT timestamp file.

        Current self.segments = predictions. User selects GT annotations.
        The report is written to xlsx/csv/tsv with signed errors:
        ...
        if not self.segments:
            QMessageBox.warning(
                self, "No predictions",
                "Run Auto-Detect / Align GT first, or load a prediction file, then validate."
            )
            return

        if _XLSX:
            filt = ("GT annotations (*.csv *.tsv *.xlsx *.xls);;"
                    "Excel (*.xlsx *.xls);;CSV (*.csv);;TSV (*.tsv);;All Files (*)")
        else:
            filt = "GT annotations (*.csv *.tsv);;CSV (*.csv);;TSV (*.tsv);;All Files (*)"

        gt_path, _ = QFileDialog.getOpenFileName(
            self, "Select Ground Truth timestamp file", str(OUTPUT_DIR), filt,
        )
        if not gt_path:
            return

        try:
            gt_segments, skipped = _load_annotation_segments(gt_path)
        except Exception as e:
            traceback.print_exc()
        ...
        if not gt_segments:
            QMessageBox.warning(self, "Empty GT", "No valid GT rows found. Expected: label,start,end[,prediction].")
            return

        iso, _ = self._current_lang()
        file_sentences, file_paragraph = load_transcripts_file(iso)
        # Prefer imported/current text material if available, then transcripts/<iso>.txt
        sentences = self.current_sentences or file_sentences
        paragraph = self.current_paragraph or file_paragraph

        gt_units = _gt_units_from_segments(gt_segments, sentences=sentences, paragraph=paragraph)
        pred_units = _pred_units_from_segments(self.segments)
        rows = _compare_units(gt_units, pred_units)

        if not rows:
            QMessageBox.warning(self, "No report", "Could not build validation rows.")
            return

        default_name = "level_validation_report.xlsx" if _XLSX else "level_validation_report.csv"
        out_path, _ = QFileDialog.getSaveFileName(
            self, "Save level-wise validation report",
            str(OUTPUT_DIR / default_name),
            "Excel (*.xlsx);;CSV (*.csv);;TSV (*.tsv);;All Files (*)",
        )
        if not out_path:
            return
        ...
        header = [
            ...
            "gt_start", "gt_end",
            "pred_start", "pred_end",
            "start_error_sec(pred-gt)",
            "end_error_sec(pred-gt)",
            "duration_error_sec(pred-gt)",
            "text_similarity",
            "status",
            "note",
        ]
        ...
        try:
            _write_table(out_path, rows, header=header)
            counts = {}
            for r in rows:
                counts[r[12]] = counts.get(r[12], 0) + 1
            partial = counts.get("PARTIAL_PARAGRAPH", 0)
            extra = f" │ partial paragraph: {partial}" if partial else ""
            self._set_status(
                f"✓ Validation report saved → {out_path} │ rows={len(rows)} │ "
                f"CHECK={counts.get('CHECK',0)} MISSING={counts.get('MISSING_PRED',0)} EXTRA={counts.get('EXTRA_PRED',0)}{extra}"
            )
            QMessageBox.information(
                self, "Validation complete",
                "Saved level-wise report.\n\n"
                "Timestamp error convention:\n"
                "  +ve = predicted timestamp is later/longer than GT\n"
                "  -ve = predicted timestamp is earlier/shorter than GT\n\n"
                f"Rows: {len(rows)}\nOutput: {out_path}"
            )
        except Exception as e:
            traceback.print_exc()
            QMessageBox.critical(self, "Save report error", str(e))
    ...
    def on_load_csv(self):
        ...
        loaded, skipped = [], 0
        # Detect & skip an obvious header row (e.g. "label, start, end" from xlsx)
        if rows:
            first = [str(c).strip().lower() for c in rows[0]]
            if (len(first) >= 3 and
                "label" in first[0] and
                ("start" in first[1] or "begin" in first[1]) and
                "end" in first[2]):
                rows = rows[1:]
